...
import { Play, Pause, StepForward, StepBack, RefreshCw, Clock, Terminal, BookOpen } from 'lucide-react';
import { ALGORITHM_SCENARIOS, AlgorithmScenario, VisualState } from '../data/algorithm_steps';
import { 
    VisualInsertConcept, 
    VisualSearchConcept, 
    VisualStartsWithConcept, 
    VisualDeleteConcept, 
    VisualBasicConcept 
} from './VisualConcepts';
...
export const AlgorithmLab: React.FC = () => {
  ...
  // Helper to render the specific concept visual
  const renderConceptVisual = () => {
      switch(activeScenarioId) {
          case 'BASIC': return <VisualBasicConcept />;
          case 'INSERT': return <VisualInsertConcept />;
          case 'SEARCH': return <VisualSearchConcept />;
          case 'STARTSWITH': return <VisualStartsWithConcept />;
          case 'DELETE': return <VisualDeleteConcept />;
          default: return null;
      }
  };

  return (
    <div className="space-y-8 animate-fade-in w-full h-full pb-8 relative z-10">
      {/* 1. Header & Concept Deck */}
      <div className="space-y-6 text-center">
        <h2 className="text-5xl font-serif font-bold text-[#433422] drop-shadow-sm">The Scribe's Desk</h2>
        <p className="text-xl text-[#78716c] font-serif italic max-w-2xl mx-auto">
           "To write the code is to trace the path of thought."
        </p>
        
        <div className="flex flex-wrap justify-center gap-4 mt-8">
           {Object.values(ALGORITHM_SCENARIOS).map(s => (
             <button
               key={s.id}
             ...
             <div className="p-8 flex flex-col md:flex-row gap-12 text-left min-h-[400px]">
                ...
                <div className="md:w-1/2 flex flex-col space-y-4">
                    ...
                    <div className="bg-white p-6 rounded-lg border border-[#e7e5e4] flex flex-col justify-center items-center text-center shadow-sm">
                        ...
                        <div className="text-xs uppercase tracking-widest text-[#9a3412] font-bold mb-2">Time Complexity</div>
                        <div className="text-2xl font-bold font-mono text-[#433422]">{scenario.complexity}</div>
                        <p className="text-xs text-[#78716c] mt-2 italic font-serif">Independent of dictionary size.</p>
                    </div>
                </div>
             </div>
        </div>
      ...
      {/* 2. The Workbench (Split Screen) - Expanded to Full Viewport Height */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 h-[85vh] min-h-[900px]">
         ...
         <div className="bg-[#1e1e1e] rounded-xl shadow-2xl overflow-hidden flex flex-col border border-[#333] h-full">
            <div className="bg-[#252526] px-4 py-2 flex items-center justify-between border-b border-[#333] shrink-0">
               ...
               <div className="text-gray-400 text-xs font-mono">trie_algorithm.py</div>
            </div>
            ...
            <div className="p-6 overflow-auto flex-1 font-mono text-sm leading-8 scrollbar-thin scrollbar-thumb-[#444] scrollbar-track-[#1e1e1e]">
               {scenario.code.split('\n').map((line, idx) => {
                 ...
                 return (
                   <div 
                     key={idx} 
                     className={`flex transition-colors duration-200 ${isActive ? 'bg-[#37373d]' : ''}`}
                   >
         ...
         <div className="bg-[#fffbeb] rounded-xl shadow-2xl border border-[#e7e5e4] relative overflow-hidden flex flex-col h-full">
            ...
            <div className="relative z-20 bg-white border-b border-[#d97706]/20 p-6 min-h-[160px] flex flex-col justify-center shadow-sm shrink-0">
               <div className="flex items-start gap-4">
                  <div className="bg-[#fff7ed] p-2 rounded-lg border border-[#fed7aa] mt-1 shrink-0">
                      <Terminal className="text-[#d97706]" size={20} />
                  </div>
                  <div className="flex-1">
                    <span className="text-xs uppercase font-bold text-[#9a3412] tracking-wider block mb-2">Execution Console</span>
                    <p className="text-[#433422] font-serif text-lg leading-snug animate-fade-in key={currentStep}">
                        {stepData.log}
                    </p>
                  </div>
               </div>
            ...
            <div className="bg-white border-t border-[#e7e5e4] p-4 flex items-center justify-between z-20 shrink-0">
               <div className="flex items-center space-x-2">
                 <button onClick={reset} className="p-2 hover:bg-[#f3f4f6] rounded-full text-[#57534e] transition-colors" title="Reset">
                   <RefreshCw size={20} />
                 </button>
                 <div className="h-6 w-px bg-[#e7e5e4] mx-2"></div>
                 <button onClick={() => handleStep('prev')} disabled={currentStep===0} className="p-2 hover:bg-[#f3f4f6] rounded-full disabled:opacity-30 transition-colors">
                   <StepBack size={24} fill="currentColor" />
                 </button>
                 <button 
                   onClick={() => setIsPlaying(!isPlaying)} 
                   className="p-3 bg-[#433422] text-[#fdf6e3] rounded-full hover:bg-[#5c4d43] shadow-lg transform hover:scale-105 transition-all mx-2"
                 >
                   {isPlaying ? <Pause size={24} fill="currentColor" /> : <Play size={24} fill="currentColor" className="ml-1"/>}
                 </button>
                 <button onClick={() => handleStep('next')} disabled={currentStep===totalSteps-1} className="p-2 hover:bg-[#f3f4f6] rounded-full disabled:opacity-30 transition-colors">
                   <StepForward size={24} fill="currentColor" />
                 </button>
               </div>

               <div className="flex items-center space-x-6">
                  <div className="text-xs font-bold text-[#a8a29e] uppercase tracking-widest">
                    Step {currentStep + 1} of {totalSteps}
                  </div>
                  <div className="flex items-center space-x-2">
                      <span className="text-xs font-bold text-[#78716c]">SPEED</span>
                      <select 
                        value={speed} 
                        onChange={(e) => setSpeed(Number(e.target.value))}
                        className="bg-[#f5f5f4] border border-[#d6d3d1] text-[#57534e] text-xs rounded py-1 px-2 outline-none cursor-pointer focus:ring-1 focus:ring-[#d97706]"
                      >
...
const MarkdownRenderer = ({ content }: { content: string }) => {
    // 1. Normalize headers: Ensure they are surrounded by double newlines so they split cleanly.
    // Regex matches "### Text" regardless of what is before it.
    ...
    return (
        <div className="space-y-4 font-serif text-lg text-[#57534e]">
            {blocks.map((block, i) => {
                ...
                if (block.startsWith('###')) {
                    return (
                        <h3 key={i} className="text-2xl font-bold text-[#433422] mt-6 mb-3 border-b-2 border-[#d97706]/20 pb-2 flex items-center">
                ...
                if (block.startsWith('*') || block.startsWith('-')) {
                    ...
                    return (
                       <ul key={i} className="list-disc pl-6 space-y-2 mb-4 bg-[#fff7ed]/50 p-4 rounded-lg border border-[#fed7aa]/30">
                           {items.map((item, j) => (
                               <li key={j} dangerouslySetInnerHTML={{__html: formatText(item.replace(/^[\*\-]/, '').trim())}} />
                           ))}
                       </ul>
                   )
                }
                ...
                if (/^\d+\./.test(block)) {
                     ...
                     return (
                        <ol key={i} className="list-decimal pl-6 space-y-3 mb-4 bg-[#f0fdf4]/30 p-4 rounded-lg border border-[#bbf7d0]/30">
                            {items.map((item, j) => (
                                <li key={j} className="pl-2" dangerouslySetInnerHTML={{__html: formatText(item.replace(/^\d+\./, '').trim())}} />
                            ))}
                        </ol>
                     );
                }
...
// Basic text formatter for **bold** and `code`
const formatText = (text: string) => {
    return text
      .replace(/\*\*(.*?)\*\*/g, '<strong class="text-[#d97706] font-bold">$1</strong>')
      .replace(/`([^`]+)`/g, '<code class="bg-[#f5f5f4] px-1.5 py-0.5 rounded text-[#d97706] text-sm font-bold font-mono border border-[#e7e5e4]">$1</code>');
}
  ...
  return (
    ...
    <svg viewBox="0 0 800 500" className="w-full h-full transition-all duration-500">
       ...
       {state.nodes.map(node => {
          ...
          if (node.status === 'ghost') { fill = '#fecaca'; stroke = RED; textColor = RED; }
          return (
            <g key={node.id} className="transition-all duration-500">
               ...
               {node.status === 'ghost' && <text x={node.x} y={node.y - 35} textAnchor="middle" fontSize="20">✂️</text>}
            </g>
          );
       })}
